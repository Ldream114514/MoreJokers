...
SMODS.Joker {
   ...
   perishable_compat = false,
   eternal_compat = true,
   config = { extra = { chips = 0, chips_mod = 2, chips_gain = 0 } },
   rarity = 2,
   atlas = 'morejokers',
	pos = { x = 9, y = 1 },
	cost = 6,
...
SMODS.Joker {
    ...
    cost = 7,
    pos = { x = 1, y = 2 },
    config = { extra = { chips_mod = 0.1, chips = 1 } },
    loc_vars = function(self, info_queue, card)
...
SMODS.Joker {
    ...
    cost = 7,
    pos = { x = 6, y = 4 },
    config = { extra = { Xmult = 1, Xmult_gain = 0.75, Xmult_loss = 0.1 } },
    loc_vars = function(self, info_queue, card)
    ...
    if card.ability.extra.Xmult > 1 and context.pseudorandom_result and not context.result and context.identifier == 'wheel_of_fortune' and not context.blueprint then 
        card.ability.extra.Xmult = card.ability.extra.Xmult - card.ability.extra.Xmult_loss
        return {
                message = localize { type = 'variable', key = 'a_xmult_minus', vars = { card.ability.extra.Xmult_loss } }
                }
...
SMODS.Joker {
    ...
    pos = { x = 7, y = 3 },
    calculate = function(self, card, context)
    if context.cardarea == G.jokers and context.end_of_round and G.GAME.current_round.discards_used == 0 and G.GAME.current_round.discards_left > 0 then
